...
NOMES={"1":"1","2":"2","5":"5","10":"10","CoinFlip":"Coin Flip","CashHunt":"Cash Hunt","Pachinko":"Pachinko","CrazyBonus":"Crazy Time"}
CORES={"1":"#64748b","2":"#64748b","5":"#64748b","10":"#64748b","CoinFlip":"#3b82f6","CashHunt":"#22c55e","Pachinko":"#a855f7","CrazyBonus":"#ef4444"}
PASTA=os.path.join(os.path.dirname(os.path.abspath(__file__)),"Logs"); os.makedirs(PASTA, exist_ok=True)
STATE_FILE=os.path.join(PASTA,"crazy_time_state.json")
LOG_FILE=os.path.join(PASTA,"crazy_time_combo_log.txt")
...
class App(ctk.CTk):
    def __init__(self, in_q, out_q):
        ...
        bombear(self)
        self.title("CRAZY TIME — Pipeline 24reqs")
        self.geometry("1120x820"); ctk.set_appearance_mode("dark")
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(h, text="CRAZY TIME", font=("Arial",20,"bold"), text_color="#c084fc").pack(side="left", padx=10)
        ...
        # Logo
        try:
            self._logo_img = _load_logo_image("crazy_time")
            if self._logo_img:
        ...
        box=ctk.CTkFrame(mid, fg_color="#2e1065", border_width=2, border_color="#c084fc")
        box.pack(side="left", fill="both", expand=True, padx=(0,6))
        ctk.CTkLabel(box, text="PREVISÃO (LSTM ∩ fonte)", font=("Arial",13,"bold"), text_color="#fde68a").pack(pady=5)
        row=ctk.CTkFrame(box, fg_color="transparent"); row.pack(fill="x", padx=8, pady=8)
        self.cards=[]
        for _ in range(3):
            f=ctk.CTkFrame(row, fg_color="#0f172a", border_width=2, border_color="#c084fc")
            f.pack(side="left", expand=True, fill="both", padx=4)
            nome=ctk.CTkLabel(f, text="—", font=("Arial",16,"bold")); nome.pack(pady=15)
            self.cards.append(nome)
        ...
        ctk.CTkLabel(hist_box, text="HISTÓRICO", font=("Arial",13,"bold")).pack(pady=(6,2))
        ...
        ctk.CTkLabel(self, text="Auditoria", font=("Arial",13,"bold")).pack(anchor="w", padx=15, pady=(10,0))
    ...
    def desenhar_hist(self, rows):
        ...
        for r in rows[:15]:
            sec=r.get("sec") or r.get("n"); key=str(r.get("settled") or sec)
            c=ctk.CTkFrame(self.hist_scroll, fg_color="#0f172a"); c.pack(side="left", padx=3, pady=4)
            ctk.CTkLabel(c, text=NOMES.get(sec,sec), width=84, height=32, corner_radius=6,
                         fg_color=CORES.get(sec,"#64748b"), text_color="#fff", font=("Arial",11,"bold")).pack(side="left", padx=3, pady=5)
            if key in self._acertos_keys:
                ctk.CTkLabel(c, text="✓", text_color="#22c55e", font=("Arial",16,"bold")).pack(side="left", padx=3)
            elif key in self._erros_keys:
                ctk.CTkLabel(c, text="✗", text_color="#ef4444", font=("Arial",16,"bold")).pack(side="left", padx=3)

    def validar(self, r):
        """Só estado — UI atualizada na thread principal via _aplicar_validacao_ui."""
        s=r.get("sec") or r.get("n")
        ...
        if self.restantes <= 0:
            ...
            if janela_hit:
                ...
                try:
                    ...
                    pass
                self.last_result={"saiu": s, "acertou": True, "settled": r.get("settled"), "window_done": True}
                self._ui_validacao = ("ok", s, 0)
                log(f"JANELA OK | último={s} | ok={self.ok} err={self.err}")
            else:
                self.err += 1
                try:
                    from metricas_honestas import p_esperado_para
                    ...
                    pass
                self.last_result={"saiu": s, "acertou": False, "settled": r.get("settled"), "window_done": True}
                self._ui_validacao = ("err_fim", s, 0)
                log(f"JANELA ERRO | último={s} | ok={self.ok} err={self.err}")
            self.escolhas=[]
            self._janela_hit = False
        else:
            if hit:
                self._janela_hit = True
            self.last_result={"saiu": s, "acertou": hit, "settled": r.get("settled"), "window_done": False}
            self._ui_validacao = ("hit" if hit else "err", s, self.restantes)
    ...
    def _aplicar_validacao_ui(self):
        ...
        try:
            ...
            self._atualizar_placar_num()
        if tipo == "ok":
            self.res.configure(text=f"✅ JANELA ACERTO | último {NOMES.get(s,s)}", text_color="#22c55e")
            self.janela_lbl.configure(text="Janela: —")
        elif tipo == "err_fim":
            self.res.configure(text=f"❌ JANELA ERRO | último {NOMES.get(s,s)}", text_color="#ef4444")
            self.janela_lbl.configure(text="Janela: —")
        elif tipo == "hit":
            self.res.configure(text=f"✅ HIT {NOMES.get(s,s)} — restam {rest}", text_color="#86efac")
            self.janela_lbl.configure(text=f"Janela: {rest}")
        else:
            self.res.configure(text=f"❌ MISS {NOMES.get(s,s)} — restam {rest}", text_color="#fca5a5")
            self.janela_lbl.configure(text=f"Janela: {rest}")
        self._ui_validacao = None
    ...
    def aplicar(self, sug):
        ...
        if self.escolhas and self.restantes > 0:
            ...
            for i,l in enumerate(self.cards):
                l.configure(text=NOMES.get(self.escolhas[i], self.escolhas[i]) if i < len(self.escolhas) else "—")
        ...
        try:
            self._render_hist(getattr(self, "last_nums", []) or [])
        except Exception:
            pass
    ...
    def _render_hist(self, nums):
        try:
            for w in self.hist_scroll.winfo_children():
                w.destroy()
            for n in list(nums)[:40]:
                ctk.CTkLabel(self.hist_scroll, text=str(n), width=48, height=28,
                             fg_color="#334155", corner_radius=6).pack(side="left", padx=2, pady=4)
        except Exception:
            pass
    ...
    def worker(self):
        ...
        try:
            ...
            if mudou:
                self.validar({"sec": rows[0].get("sec") or rows[0].get("n"), "settled": rows[0].get("settled"), "n": rows[0].get("n")})
            ...
            payload={
                "nums": [x.get("sec") or x.get("n") for x in rows],
...
def main():
    mp.freeze_support()
    # FILAS COM TETO: sem limite, um cerebro engasgado acumula pedido a
    # cada volta -- e cada pedido leva 200 linhas com as tags.
    in_q,out_q=mp.Queue(maxsize=4),mp.Queue(maxsize=8)
    p=mp.Process(target=process_cerebro, args=(in_q,out_q,"crazy_time"), daemon=True); p.start()
    try:
        app = App(in_q,out_q)
