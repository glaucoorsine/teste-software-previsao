...
def _logo_path(jogo="immersive"):
    from pathlib import Path
    root = Path(__file__).resolve().parent / "logos"
    for name in (f"{jogo}_logo.png", f"{jogo}_logo.jpg", f"{jogo}_0.jpg"):
        p = root / name
        if p.is_file():
            return str(p)
    return None
...
# -*- coding: utf-8 -*-
"""IMMERSIVE — settled+mults+last_result no worker | placar acertos/erros | horizonte fixo | previsibilidade acadêmica."""
import customtkinter as ctk
...
GAME = "immersive"
API = "https://api-cs.casino.org/svc-evolution-game-events/api/immersiveroulette"
HEADERS = {
...
class App(ctk.CTk):
    def __init__(self, in_q, out_q):
        ...
        self.title("IMMERSIVE — Pipeline 24reqs")
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(h, text="IMMERSIVE", font=("Arial",20,"bold"), text_color="#38bdf8").pack(side="left", padx=10)
        ...
        # Logo
        try:
            self._logo_img = _load_logo_image("immersive")
            if self._logo_img:
...
def main():
    ...
    p=mp.Process(target=process_cerebro, args=(in_q, out_q, "immersive"), daemon=True); p.start()
